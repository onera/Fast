...
!     write(*,'(a18,7i5)')'init_ssiter impli',ind_dm(:,nd,nitcfg),nitcfg
      ...
      ELSE IF (param_int(EXPLOC).eq.1.and.param_int(RK).eq.2 
     & .and.param_int(ITYPCP)==2) THEN ! Explicit local 
...
c$$$         ELSE IF (MOD(nitcfg,ind)==2) THEN         
c$$$            
c$$$            IF (param_int(LEVELG)>param_int(LEVEL).AND.
c$$$     &           param_int(LEVEL)>param_int(LEVELD) ) THEN
c$$$               
c$$$               ndfin = ndfin + 1
c$$$
...
c$$$
c$$$            ELSE IF (param_int(LEVELD)>param_int(LEVEL).AND.
c$$$     &              param_int(LEVEL)>param_int(LEVELG) ) THEN
c$$$               
c$$$               ndfin = ndfin + 1
c$$$
...
c$$$            END IF
      ...
      ELSE IF (param_int(EXPLOC).eq.5.and.param_int(RK).eq.2
     & .and.param_int(ITYPCP)==2) THEN !Tang & Warnecke  
...
!!!   renvoie 2 colonnes pour le calcul du predicteur pour la zone de niveau superieur !!!
     ...
     &              param_int(LEVEL)>param_int(LEVELG) ) THEN
      ...
      ELSE IF (param_int(EXPLOC).eq.2.and.param_int(ITYPCP)==2.and.
     & param_int(RK).eq.3 ) THEN ! schéma pas de temps local d'ordre 3 
