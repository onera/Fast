...
c     $Author: IvanMary $
c***********************************************************************
      subroutine cplevel( param_int, param_real,
     &                  ind_loop,
     &                  level,
...
#include   "FastS/Compute/loop_begin.for"
     ...
     &                  +tj(lt+incj,2)*tj(lt+incj,2) )
              ...
              !level(l) = int(1./cfloc)
              level(l) = (1./cfloc)
              !write(*,*)'level',xinvspc,l-lij+ind_loop(1),j
#include   "FastS/Compute/loop_end.for"
...
CCC Mise a jour valeur unique sur sous bloque
#include   "FastS/Compute/loop_begin.for"
              level(l) = int(1./cfloc)
#include   "FastS/Compute/loop_end.for"
