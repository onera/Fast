...
PyObject* K_FASTS::dtlocal2para_mpi(PyObject* self, PyObject* args)
{
  ...
  //cout << "taille_tabs= " << taille_tabs << endl;
 ...
 /// Recuperation du tableau de stockage des valeurs
  ...
  //E_Float** iptcstk_d = new E_Float*[2];
  //iptcstk_d[0] = iptcstk;
  //iptcstk_d[1] = iptcstk + taille_tabs/2;
...
#pragma omp parallel default(shared) //private(cycle)
  {
	...
	//if (NoTransfert==20){cout << "process,nrac= " << process <<" "<< nrac << endl;}

	for  (E_Int irac=0; irac< nrac; irac++) // Boucle sur les différents raccords (frontières)

	  {

	    //cout << "irac= " << irac <<" "<< process << endl;
		...
		if (nstep%cycle==1 and (nstep/cycle)%2==0) /// La 1ere sous-iteration du cycle
		  {
		    ...
		    //cout << "process_ap= " << process << endl;
		    
		    //if (NoTransfert==20 and irac==nrac-1){cout << "= process, NoD, NoR= " << process <<" "<< ipt_param_int[debut_rac + 25] <<" "<< ipt_param_int[debut_rac + 24]  << endl;}
		...
		if (nstep%cycle == 1)

		  {
		    ...
		    //cout << donorPts[0]<<"  "<< donorPts[1]<<"  "<< donorPts[2]<<"  "<< donorPts[3]<<"  "<< NoD <<"  "<< process << endl; 
		...
		if (nstep%cycle == cycle/4)
		  ...
		  {		  
		    ...
		    /// Interpolation de y2
		    // interp_rk3local3para_(param_intt[NoD], param_realt[NoD], iptcoe+shift_coe[NoD], donorPts, donorPts_, iptstk + irac*taille + 0*taillefenetre,iptstk + irac*taille + 1*5*taillefenetre, iptro[NoD],dir,taillefenetre,nstep,process);
		...
		if (nstep%cycle == cycle/2 + 1)

		  {
		    ...
		    //Interpolation de y6
		    //interp_rk3local3para_(param_intt[NoD], param_realt[NoD], iptcoe+shift_coe[NoD], donorPts,donorPts_, iptstk + irac*taille + 0*taillefenetre,iptstk + irac*taille + 1*5*taillefenetre, iptro_p1[NoD],dir,taillefenetre,nstep,process);
...
#pragma omp parallel default(shared) //private(cycle)
  {
	...
	for  (E_Int irac=0; irac< nrac; irac++) // Boucle sur les différents raccords (frontières)

	  {
	    ...
	    //if (NoTransfert==1 and irac==0){cout << "process, NoD, NoR= " << process <<" "<< ipt_param_int[debut_rac + 25] <<" "<< ipt_param_int[debut_rac + 24] << endl;}
		...
		if (nstep%cycle == 1 )//and irac == 5 and process == 0)

		  {
		    ...
		    //if  (donorPts_[0] ==  199 and  donorPts_[1] == 200 and donorPts_[2] == 1 and donorPts_[3] == 101 and process == 0 and NoD == 1)
		    // {
		    //cout << "irac= "<< irac <<" "<<NoTransfert<<" "<< process << endl;		     
		    //  }

		    //cout << donorPts[0]<<"  "<< donorPts[1]<<"  "<< donorPts[2]<<"  "<< donorPts[3]<<"  "<< NoD <<"  "<< process << endl; 

		 
		    //cout << "yn= " << *(iptstk + irac*taille + 0*taillefenetre + 200)<<" "<<"y1= "<< *(iptstk + irac*taille + 1*5*taillefenetre + 200)<<" "<< NoD <<"  "<<endl;// process << endl
		    //if (irac==3 and process==0 and NoTransfert==2 or irac==5 and process==0)//irac==3 and process==0 and NoTransfert==1 )
			// {
			// cout << "coucou" <<" "<< debut_rac <<"  "<< NoTransfert << endl;
			// } //cout << "coucou" << " " << process << endl;
...

	 {
	   ...
	   if (ipt_param_int[debut_rac + 25] < ipt_param_int[debut_rac + 24]) // Le pas de temps de la zone donneuse est plus grand que celui de la zone receveuse 
	       ...
	       if (nstep%cycle==1 or nstep%cycle==cycle/2 or nstep%cycle==cycle-1)
		 {
		   ...
		   //cout << "receveurs= " << donorPts_[0]<<" "<< donorPts_[1] <<" "<< donorPts_[2]<<" "<< donorPts_[3] << endl;
		   //cout << "ind= " << ind << endl;
		   conservrk3local3para_(param_intt[NoD],donorPts,donorPts_,iptdrodm + shift_zone[NoD],iptcoe+shift_coe[NoD], iptcstk+irac*taille,taillefenetreD,nstep,ind);
