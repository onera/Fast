...
#include  "FastS/Compute/Normale/normale_3dhomo_k.for"

        nm  = l -  inck
        nm2 = l -2*inck
        np  = l +  inck

#include  "FastS/Compute/SENSOR_INIT/wiggle_k.for"
...
#include "FastS/Compute/Vit_ent/qn_ale_3dhomo_k.for"

        ! modification de vitesse normale par ajout
        ! de stabilisation de type Rhie-Chow
        u  = 0.25*(qn1+qn2)- c2*sk*(p2-p1)*(opt0*wig( l+ wig_k)+1.-opt0)

        tdu = max(abs(u),c1*sk)*wig( l+ wig_k)

        !Calcul du flux total
        p1p2= (p1+p2)*0.5

#include "FastS/Compute/Vit_ent/fluvector_ale_3dhomo_k.for"
