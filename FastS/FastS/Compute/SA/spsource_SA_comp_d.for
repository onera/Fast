...
C
C  Differentiation of spsource_sa_comp in forward (tangent) mode:
...
C     $Author: IvanMary $
C***********************************************************************
      SUBROUTINE SPSOURCE_SA_COMP_D(ndom, nitcfg, param_int, param_real
     +                              , ind_loop, xmut, rop, ropd, coe, ti
     +                              , tj, tk, vol, dlng, drodm, drodmd)
      IMPLICIT NONE
C
...
C
     ...
     +       , dvdx, dvdy, dvdz, dwdx, dwdy, dwdz, gam, rgp, cc
...
C
Cdudy
                  dudy = u3*tix1 - u1*tix + u2*tjx1 - u4*tjx + u5*tkx1 -
     +              u6*tkx
C
Cdudz
...
C
Cdvdy
                  dvdx = u3*tix1 - u1*tix + u2*tjx1 - u4*tjx + u5*tkx1 -
     +              u6*tkx
C
Cdvdz
                  dvdxd = tiy1*u3d - tiy*u1d + tjy1*u2d - tjy*u4d + tky1
     +              *u5d - tky*u6d
                  dvdx = u3*tiy1 - u1*tiy + u2*tjy1 - u4*tjy + u5*tky1 -
     +              u6*tky
C
                  dvdzd = tiz1*u3d - tiz*u1d + tjz1*u2d - tjz*u4d + tkz1
     +              *u5d - tkz*u6d
                  dvdz = u3*tiz1 - u1*tiz + u2*tjz1 - u4*tjz + u5*tkz1 -
     +              u6*tkz
C
...
C
Cdwdy
                  dwdxd = tix1*u3d - tix*u1d + tjx1*u2d - tjx*u4d + tkx1
     +              *u5d - tkx*u6d
                  dwdx = u3*tix1 - u1*tix + u2*tjx1 - u4*tjx + u5*tkx1 -
     +              u6*tkx
C
Cdwdz
                  dwdy = u3*tiy1 - u1*tiy + u2*tjy1 - u4*tjy + u5*tky1 -
     +              u6*tky
C
                  dwdyd = tiy1*u3d - tiy*u1d + tjy1*u2d - tjy*u4d + tky1
     +              *u5d - tky*u6d
                  dwdy = u3*tiy1 - u1*tiy + u2*tjy1 - u4*tjy + u5*tky1 -
     +              u6*tky
C
...
Cformulation compressible complete
                  ccd = -((3.5*(ropd(l, 1)*rop(l, 6)**2+rop(l, 1)*2*rop(
     +              l, 6)*ropd(l, 6))*gam*rgp*rop(l, 5)-3.5*rop(l, 1)*
     +              rop(l, 6)**2*gam*rgp*ropd(l, 5))*(dudx**2+dudy**2+
     +              dudz**2+dvdx**2+dvdy**2+dvdz**2+dwdx**2+dwdy**2+dwdz
     +              **2)/(gam**2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*rop(
     +              l, 6)**2*(2*dudx*dudxd+2*dudy*dudyd+2*dudz*dudzd+2*
     +              dvdx*dvdxd+2*dvdz*dvdzd+2*dwdx*dwdxd+2*dwdy*dwdyd)/(
     +              gam*rgp*rop(l, 5)))
...
Cdudz
     ...
     +              tkz)
C
                  anviscd = xvol**2*(c1*(dudxd+dudyd+dudzd)+ccd)
...
C
Cformulation compressible complete
                ccd = -((3.5*(ropd(l, 1)*rop(l, 6)**2+rop(l, 1)*2*rop(l
     +            , 6)*ropd(l, 6))*gam*rgp*rop(l, 5)-3.5*rop(l, 1)*rop(l
     +            , 6)**2*gam*rgp*ropd(l, 5))*(dudx**2+dudy**2+dudz**2+
     +            dvdx**2+dvdy**2+dvdz**2+dwdx**2+dwdy**2+dwdz**2)/(gam
     +            **2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*rop(l, 6)**2*(2
     +            *dudx*dudxd+2*dudy*dudyd+2*dudz*dudzd+2*dvdx*dvdxd+2*
     +            dvdz*dvdzd+2*dwdx*dwdxd+2*dwdy*dwdyd)/(gam*rgp*rop(l, 
     +            5)))
                cc = -(3.5*rop(l, 1)*rop(l, 6)**2/(gam*rgp*rop(l, 5))*(
     +            dudx**2+dudy**2+dudz**2+dvdx**2+dvdy**2+dvdz**2+dwdx**
     +            2+dwdy**2+dwdz**2))
C
                u1d = ropd(l, 6) + ropd(l2, 6)
                u1 = rop(l, 6) + rop(l2, 6)
...
C
Cdvdy
                  dvdxd = tix1*u3d - tix*u1d + tjx1*u2d - tjx*u4d
                  dvdx = u3*tix1 - u1*tix + u2*tjx1 - u4*tjx
C
Cdvdz
                  dvdyd = tiy1*u3d - tiy*u1d + tjy1*u2d - tjy*u4d
...
C
Cdwdy
                  dwdxd = tix1*u3d - tix*u1d + tjx1*u2d - tjx*u4d
                  dwdx = u3*tix1 - u1*tix + u2*tjx1 - u4*tjx
C
Cdwdz
                  dwdyd = tiy1*u3d - tiy*u1d + tjy1*u2d - tjy*u4d
...
Cformulation compressible complete
     ...
     +              **2)/(gam**2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*rop(
     +              l, 6)**2*(2*dudx*dudxd+2*dudy*dudyd+2*dudz*dudzd+2*
     +              dvdx*dvdxd+2*dvdy*dvdyd+2*dvdz*dvdzd+2*dwdx*dwdxd+2*
     +              dwdy*dwdyd+2*dwdz*dwdzd)/(gam*rgp*rop(l, 5)))
                  cc = -(3.5*rop(l, 1)*rop(l, 6)**2/(gam*rgp*rop(l, 5))*
     +              (dudx**2+dudy**2+dudz**2+dvdx**2+dvdy**2+dvdz**2+
     +              dwdx**2+dwdy**2+dwdz**2))
C
...
Cformulation compressible complete
     ...
     +            **2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*rop(l, 6)**2*(2
     +            *dudx*dudxd+2*dudy*dudyd+2*dudz*dudzd+2*dvdx*dvdxd+2*
     +            dvdy*dvdyd+2*dvdz*dvdzd+2*dwdx*dwdxd+2*dwdy*dwdyd+2*
     +            dwdz*dwdzd)/(gam*rgp*rop(l, 5)))
                cc = -(3.5*rop(l, 1)*rop(l, 6)**2/(gam*rgp*rop(l, 5))*(
     +            dudx**2+dudy**2+dudz**2+dvdx**2+dvdy**2+dvdz**2+dwdx**
     +            2+dwdy**2+dwdz**2))
C
...
Cformulation compressible complete
     ...
     +              **2)/(gam**2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*rop(
     +              l, 6)**2*(2*dudx*dudxd+2*dudy*dudyd+2*dudz*dudzd+2*
     +              dvdx*dvdxd+2*dvdy*dvdyd+2*dvdz*dvdzd)/(gam*rgp*rop(l
     +              , 5)))
                  cc = -(3.5*rop(l, 1)*rop(l, 6)**2/(gam*rgp*rop(l, 5))*
     +              (dudx**2+dudy**2+dudz**2+dvdx**2+dvdy**2+dvdz**2+
     +              dwdx**2+dwdy**2+dwdz**2))
...
Cformulation compressible complete
     ...
     +            **2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*rop(l, 6)**2*(2
     +            *dudx*dudxd+2*dudy*dudyd+2*dudz*dudzd+2*dvdx*dvdxd+2*
     +            dvdy*dvdyd+2*dvdz*dvdzd)/(gam*rgp*rop(l, 5)))
                cc = -(3.5*rop(l, 1)*rop(l, 6)**2/(gam*rgp*rop(l, 5))*(
     +            dudx**2+dudy**2+dudz**2+dvdx**2+dvdy**2+dvdz**2+dwdx**
     +            2+dwdy**2+dwdz**2))
C
...
Cformulation compressible complete
                ccd = -((3.5*(ropd(l, 1)*rop(l, 6)**2+rop(l, 1)*2*rop(l
     +            , 6)*ropd(l, 6))*gam*rgp*rop(l, 5)-3.5*rop(l, 1)*rop(l
     +            , 6)**2*gam*rgp*ropd(l, 5))*(dudx**2+dudy**2+dvdx**2+
     +            dvdy**2)/(gam**2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*
     +            rop(l, 6)**2*(2*dudx*dudxd+2*dudy*dudyd+2*dvdx*dvdxd+2
     +            *dvdy*dvdyd)/(gam*rgp*rop(l, 5)))
                cc = -(3.5*rop(l, 1)*rop(l, 6)**2/(gam*rgp*rop(l, 5))*(
...
C
     ...
     +            tiy1*u3d-tiy*u1d+tjy1*u2d-tjy*u4d)
                dudy = dudy*(u3*tiy1-u1*tiy+u2*tjy1-u4*tjy)
C
                anviscd = xvol**2*(c1*(dudxd+dudyd)+ccd)
...
C
Cformulation compressible complete
              ccd = -((3.5*(ropd(l, 1)*rop(l, 6)**2+rop(l, 1)*2*rop(l, 6
     +          )*ropd(l, 6))*gam*rgp*rop(l, 5)-3.5*rop(l, 1)*rop(l, 6)
     +          **2*gam*rgp*ropd(l, 5))*(dudx**2+dudy**2+dvdx**2+dvdy**2
     +          )/(gam**2*rgp**2*rop(l, 5)**2)+3.5*rop(l, 1)*rop(l, 6)**
     +          2*(2*dudx*dudxd+2*dudy*dudyd+2*dvdx*dvdxd+2*dvdy*dvdyd)/
     +          (gam*rgp*rop(l, 5)))
              cc = -(3.5*rop(l, 1)*rop(l, 6)**2/(gam*rgp*rop(l, 5))*(
     +          dudx**2+dudy**2+dvdx**2+dvdy**2))
C
