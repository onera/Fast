...
#include  "FastS/Compute/Normale/normale_3dfull_j.for"
...
#include  "FastS/Compute/SENSORHYPER/wiggle_j.for"
...
#include "FastS/Compute/Vit_ent/qn_ale_3dfull_j.for"

       ! modification de vitesse normale par ajout
       ! de stabilisation de type Rhie-Chow
       us  = 0.25*(qn1+qn2)- (1.-wig(l+wigd))*(
     &  c2*sj*(p2-p1)*(opt0*wig( l+ wig_j)+1.-opt0))
       tdu = max(abs(us),c1*sj)*wig( l+ wig_j)
       !Calcul du flux total
       p1p2= (p1+p2)*0.5

#include "FastS/Compute/Vit_ent/fluhyper_ale_3dfull_j.for"
