...
//printf("nidomMM %d %d %d %d %d %d \n",  nidom  , nssiter , mx_sszone  , nstep, nitrun, display);

  FldArrayI tab_nijk(nidom*mx_sszone*3);   E_Int* ipt_nijk    = tab_nijk.begin();
  FldArrayI tab_ndimdx(nidom*mx_sszone);   E_Int* ipt_ndimdx  = tab_ndimdx.begin();
  FldArrayI tab_nozone(nidom*mx_sszone);   E_Int* ipt_nozone  = tab_nozone.begin();
  FldArrayI tab_nosszone(nidom*mx_sszone); E_Int* ipt_nosszone= tab_nosszone.begin();

  FldArrayI newtab_nijk(nidom*mx_sszone*3);   E_Int* ipt_nijk_new    = newtab_nijk.begin();
  FldArrayI newtab_ndimdx(nidom*mx_sszone);   E_Int* ipt_ndimdx_new  = newtab_ndimdx.begin();
  FldArrayI newtab_nozone(nidom*mx_sszone);   E_Int* ipt_nozone_new  = newtab_nozone.begin();
  FldArrayI newtab_nosszone(nidom*mx_sszone); E_Int* ipt_nosszone_new= newtab_nosszone.begin();
  ...
  for (E_Int c = 0; c < Nbzones; c++)
     {  
        ...
        E_Int  shift_omp    = param_int[No_zone][PT_OMP];
	...
	E_Int size_k = 0;
	    ...
	    while(lgo==1)
            ...
            for (E_Int th = 0; th < __NUMTHREADS__; th++) { param_int[No_zone][ shift_omp + th ] = -2;} //Thread inactif

            param_int[No_zone][ shift_omp + th_current                 ] =  0; //le Thread "0" (pas au sens omp: indirection entre omp_numthread et ithread)) prend le job
            //Nb threads actifs 
            param_int[No_zone][ shift_omp + __NUMTHREADS__   ] =  1;
            //topology omp
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +1] =  1;
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +2] =  1;
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +3] =  1;
            //indice sous-domaine omp

            param_int[No_zone][ shift_omp + __NUMTHREADS__ +4 ] =  ind_dm[0];
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +5 ] =  ind_dm[1];
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +6 ] =  ind_dm[2];
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +7 ] =  ind_dm[3];
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +8 ] =  ind_dm[4];
            param_int[No_zone][ shift_omp + __NUMTHREADS__ +9 ] =  ind_dm[5];


           if (display==1 && nitrun ==1) 
           {
             printf("souszone %d calcule par 1 Threads,  nstep= %d \n", c, nstep);
        ...
        else
        {
            ...
            E_Int Nthreads =  size_c/cells_tg;
...
*/
		  ...
		  E_Int th_current = 0;
	          ...
	          while(size_loc/marge > remaind[th_current] && th_current < __NUMTHREADS__-1) th_current +=1;
                  ...
                  param_int[No_zone][ shift_omp + th_current ] = th;                                    //Thread actif
            ...
            //Nb threads actifs
            param_int[No_zone][ shift_omp +  __NUMTHREADS__   ] =  topo_lu[0]*topo_lu[1]*topo_lu[2];
            //topology omp
            param_int[No_zone][ shift_omp +  __NUMTHREADS__ +1] =  topo_lu[0];
            param_int[No_zone][ shift_omp +  __NUMTHREADS__ +2] =  topo_lu[1];
            param_int[No_zone][ shift_omp +  __NUMTHREADS__ +3] =  topo_lu[2];
            ...
            for (E_Int k = 0; k < topo_lu[2]; k++){
              ...
              for (E_Int j = 0; j < topo_lu[1]; j++){
                ...
                for (E_Int i = 0; i < topo_lu[0]; i++){
                    ...
                    param_int[No_zone][shift_omp + __NUMTHREADS__ +4 + th*6] = ind_dm_th[0];
                    param_int[No_zone][shift_omp + __NUMTHREADS__ +5 + th*6] = ind_dm_th[1];
                    param_int[No_zone][shift_omp + __NUMTHREADS__ +6 + th*6] = ind_dm_th[2];
                    param_int[No_zone][shift_omp + __NUMTHREADS__ +7 + th*6] = ind_dm_th[3];
                    param_int[No_zone][shift_omp + __NUMTHREADS__ +8 + th*6] = ind_dm_th[4];
                    param_int[No_zone][shift_omp + __NUMTHREADS__ +9 + th*6] = ind_dm_th[5];
           ...
           if (display==1 && nitrun ==1)
           { 
             E_Int Nthreads = param_int[No_zone][ shift_omp + __NUMTHREADS__ ];

             printf("souszone %d calcule par %d  Threads,  nstep= %d \n", c, Nthreads, nstep);

             E_Int th=0;
             for (E_Int th_current = 0; th_current < __NUMTHREADS__; th_current++){

                 if( param_int[No_zone][ shift_omp + th_current ] != -2)
                 {
                   E_Int* ind_dm = param_int[No_zone] + shift_omp + __NUMTHREADS__ +4 + th*6;
  ...
  if (display==1 && nitrun ==1)
  {  printf("OMP balancing: positive values --> slow thread. Target cells per Thread: %d \n", cells_tg_save);
     for (E_Int th_current = 0; th_current < __NUMTHREADS__; th_current++){
...
PyObject* K_FASTS::distributeThreads(PyObject* self, PyObject* args)
{
  ...
  E_Int nssiter; E_Int nstep; E_Int nitrun; E_Int mx_sszone; E_Int display;
#if defined E_DOUBLEINT
  if (!PyArg_ParseTuple(args, "OOlllll", &zones , &metrics, &nstep, &nssiter, &nitrun, &mx_sszone, &display)) return NULL; 
#else 
  if (!PyArg_ParseTuple(args, "OOiiiii", &zones , &metrics, &nstep, &nssiter, &nitrun, &mx_sszone, &display)) return NULL; 
#endif
