...
PyObject* K_FASTS::display_ss_iteration(PyObject* self, PyObject* args)
{
  ...
  { 
    ...
    if (t2 != NULL) { E_Float* cfl = K_PYTREE::getValueAF(t2, hook);
                      if (lft < 3)printf("CFL_MOY_MAX_MIN(Zone= %32.32s, nitrun=%d) %f    %f    %f \n", name, nitrun, cfl[2], cfl[0],cfl[1]); 
                    }

    if(ipt_param_int[nd][ ITYPCP ] <= 1 or (ipt_param_int[nd][ ITYPCP ] == 2 and ipt_param_int[nd][ DTLOC ]==1))
    ...
    if ( zoneCH != NULL ) { E_Int* ipt_LastRec = K_PYTREE::getValueAI(zoneCH, hook);
        ...
        conv2pytree_(cvg_ptr+2*neq*nd, nitrun, neq, ipt_LastRec, name, size_name, lft, nrec, nd,ipt_Itnum, ipt_Res_L2, ipt_Res_oo);
                          }
