...
!$OMP END SINGLE
      ...
      !on modifie les metriques (x,tijk, vol,s,vent,..) en ale en fonction du type de mvt
      IF(param_int(LALE).eq.1) THEN
       ...
       if(translation_pur.ne.0) 
     &     call cptijk_ale(ndom, param_int, ind_mtr, rot,
     &                                       ti0,tj0,tk0,ti,tj,tk)
       ...
       ind_mtr(3)=1
       ind_mtr(5)=1
       if(    (param_int(ITYPVENT).le.1.and.param_int(ITYPZONE).eq.0)
     &    .or. param_int(ITYPVENT).eq.3) then
           ind_mtr(2) = param_int( IJKV   )
           ind_mtr(4) = param_int( IJKV+1 )
           ind_mtr(6) = param_int( IJKV+2 )
       elseif(param_int(ITYPVENT).eq.1.and.param_int(ITYPZONE).eq.1)then
           ind_mtr(2) = param_int( IJKV   )
           ind_mtr(4) = param_int( IJKV+1 )
           ind_mtr(6) = 1
       else
     ...
     &                        thread_topology, ind_ven )
        ...
        if(ind_dm_omp(1).eq.ind_dm_zone(1)) 
     &    ind_ven(1)= ind_ven(1) -param_int( NIJK+3)
        if(ind_dm_omp(3).eq.ind_dm_zone(3)) 
     &    ind_ven(3)= ind_ven(3) -param_int( NIJK+3)
        if(ind_dm_omp(5).eq.ind_dm_zone(5)) 
     &    ind_ven(5)= ind_ven(5) -param_int( NIJK+4)
        if(ind_dm_omp(2).eq.ind_dm_zone(2)) 
     &    ind_ven(2)= ind_ven(2) +param_int( NIJK+3)
        if(ind_dm_omp(4).eq.ind_dm_zone(4)) 
     &    ind_ven(4)= ind_ven(4) +param_int( NIJK+3)
        if(ind_dm_omp(6).eq.ind_dm_zone(6)) 
     &    ind_ven(6)= ind_ven(6) +param_int( NIJK+4)

         !mvt de rotation: modific(ndom)ation de normale*surf
         call cpventijk_ale(ndom, param_int, param_real, ind_ven, 
     &                      x,y,z, rot, venti, ventj, ventk )


       Elseif(param_int(LALE).eq.2) Then !maillage deformable

        write(*,*)'maillage deformable non code: voir PH. Guillen'
        call error('skmtr_$',70,1)

       Else

       continue

       Endif !maillage deformable/rigid
