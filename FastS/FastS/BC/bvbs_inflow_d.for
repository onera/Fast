...
C
C  Differentiation of bvbs_inflow in forward (tangent) mode:
...
C     $Author: IvanMary $
C***********************************************************************
      SUBROUTINE BVBS_INFLOW_D(idir, lrhs, neq_mtr, param_int, ind_loop
...
C
     ...
     +          ropd(l1, 1))
              ete = rop(l1, 5)*rop(l1, 1)*param_real(2) + 0.5*(state(2)*
     +          state(2)+state(3)*state(3)+state(4)*state(4))*roe_inv
C
