...
C
C  Differentiation of bvbs_outflow in forward (tangent) mode:
...
C     $Author: IvanMary $
C***********************************************************************
      SUBROUTINE BVBS_OUTFLOW_D(idir, lrhs, neq_mtr, param_int, ind_loop
...
C Var local
     ...
     +       , eti
      REAL*8 tnx, tny, ri, roinv, sn, roe_inv
      REAL*8 ud, vd, wd, roed, rued, rved, rwed, eted, qref1d, qref2d, 
     +       qref3d, qref4d, qref5d, rd, pd, cd, cid, qvar1d, qvar2d, 
     +       qvar3d, qvar4d, qvar5d, svar1d, svar2d, svar3d, svar4d, 
     +       svar5d, rvar1d, rvar2d, rvar3d, rvar4d, rvar5d, roextd, 
     +       ruextd, rvextd, rwextd, etextd, rointd, ruintd, rvintd, 
...
Cl'etat exterieur est extrapole de l interieur sauf pour pression
              l1 = l + 1
C
              roed = ropd(l1, 1)
              roe = rop(l1, 1)
              rued = ropd(l1, 2)*roe + rop(l1, 2)*roed
              rue = rop(l1, 2)*roe
              rved = ropd(l1, 3)*roe + rop(l1, 3)*roed
              rve = rop(l1, 3)*roe
              rwed = ropd(l1, 4)*roe + rop(l1, 4)*roed
              rwe = rop(l1, 4)*roe
C
              eted = 0.5*(rued*rop(l1, 2)+rue*ropd(l1, 2)+rved*rop(l1, 3
     +          )+rve*ropd(l1, 3)+rwed*rop(l1, 4)+rwe*ropd(l1, 4))
              ete = state(5) - 0.5*(state(2)*state(2)+state(3)*state(3)+
     +          state(4)*state(4))*roe_inv + 0.5*(rue*rop(l1, 2)+rve*rop
     +          (l1, 3)+rwe*rop(l1, 4))
C
...
C
              ropd(l, 1) = ropd(l1, 1)
C
              roinvd = -(ropd(l, 1)/rop(l, 1)**2)
              roinv = 1./rop(l, 1)
C
              ropd(l, 2) = ropd(l1, 2)
              ropd(l, 3) = ropd(l1, 3)
              ropd(l, 4) = ropd(l1, 4)
              ropd(l, 5) = cvinv*(qvar5d*roinv+qvar5*roinvd-.5*(ropd(l, 
     +          2)*rop(l, 2)+rop(l, 2)*ropd(l, 2)+ropd(l, 3)*rop(l, 3)+
     +          rop(l, 3)*ropd(l, 3)+ropd(l, 4)*rop(l, 4)+rop(l, 4)*ropd
     +          (l, 4)))
C
C
              l0 = l
