...
c     $Author: IvanMary $
c***********************************************************************
      subroutine post_enst( ndo, nidom, Nbre_thread_actif, 
...
#include "FastS/HPC_LAYER/INDICE_RANGE.for"
            ...
            if(param_int(ITYPZONE).eq.0) then
               ...
               call cp_enst_3dfull(ndo, ithread, dim_grad,
     &                        param_int, c1,c2,
     &                        ind_grad,
     ...
     &                        dvardc,
     &                        rop, enst,ti,tj,tk, vol)
            

            elseif(param_int(ITYPZONE).eq.1) then

               call cp_enst_3dhomo(ndo, ithread, dim_grad,
     &                        param_int, c1,c2,
     &                        ind_grad,
     &                        ind_dm_zone, ijkv_thread, ijkv_sdm,
     &                        synchro_send_sock, synchro_send_th,
     ...
     &                        rop, enst,ti,tj,tk, vol)
            ...
            elseif(param_int(ITYPZONE).eq.2) then
               ...
               call cp_enst_3dcart(ndo, ithread,  dim_grad,
     &                        param_int, c1,c2,
     &                        ind_grad,
     &                        ind_dm_zone, ijkv_thread, ijkv_sdm,
     &                        synchro_send_sock, synchro_send_th,
     ...
     &                        rop, enst,ti,tj,tk, vol)
               ...
               call cp_enst_2d(ndo, ithread,  dim_grad,
     &                        param_int, c1,c2,
     &                        ind_grad,
     &                        ind_dm_zone, ijkv_thread, ijkv_sdm,
     &                        synchro_send_sock, synchro_send_th,
