...
if explicit_select == "explicit_local":
    filet    = DIRECTORY_CGNS+'t2DLTS.cgns'
    filetc   = DIRECTORY_CGNS+'tc2DLTS.cgns'
else:
    filet    = DIRECTORY_CGNS+'t2D.cgns'
    filetc   = DIRECTORY_CGNS+'tc2D.cgns'
...
if restart:
    if explicit_select == "explicit_local":
        filet    = DIRECTORY_CGNS+'restartLTS.cgns'
    else:
...
if explicit_select == "explicit_local":
    Fast.saveTree(t    ,fileName=DIRECTORY_CGNS+'restartLTS.cgns', split=single_vs_multiple, mpirun=isMPI )#check these files
else:
