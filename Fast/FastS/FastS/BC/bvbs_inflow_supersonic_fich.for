...
c     $Author: IvanMary $
c***********************************************************************
      subroutine bvbs_inflow_supersonic_fich(idir,lrhs, nstep,
     &                         neq_mtr, mobile_coef,
     &                         param_int, ind_loop,
     &                         param_real, c4, c5, c6,
     &                         x , y , z,
...
C...  Valeurs des grandeurs thermo imposees  (insert1)
      REAL_E ronutfich(size_data) 
      REAL_E refich(size_data) 
      REAL_E rwfich(size_data) 
      REAL_E rvfich(size_data) 
      REAL_E rufich(size_data) 
      REAL_E rofich(size_data) 
...
#include     "FastS/BC/BCInflowSupersonicFich_i.for"
...
#include       "FastS/BC/BC_nextrank_i.for"              
          ...
          do j = ind_loop(3), ind_loop(4)
             ...
             ldp  = indven( ind_loop(2)+1 , j,  k )
             lmtr = indmtr( ind_loop(2)+1 , j,  k )
#include     "FastS/BC/BCInflowSupersonicFichSA_i.for"
             do i = ind_loop(1), ind_loop(2)-1
               l    = inddm( i              , j,  k ) 
               ldp  = indven( ind_loop(2)+1 , j,  k )  !next rank
               lmtr = indmtr( ind_loop(2)+1 , j,  k )  !next rank
...
#include        "FastS/BC/BCInflowSupersonicFich_j.for"
              enddo 
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                ...
                ldp  = indven( i, ind_loop(4)+1 , k )
                lmtr = indmtr( i, ind_loop(4)+1 , k )
#include        "FastS/BC/BCInflowSupersonicFichSA_j.for"
              enddo 
            do j = ind_loop(3), ind_loop(4)-1
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
...
#include        "FastS/BC/BCInflowSupersonicFich_k.for"
              enddo 
           enddo 
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                ...
                ldp  = indven( i,      j, ind_loop(6)+1  )
                lmtr = indmtr( i,      j, ind_loop(6)+1  )
#include        "FastS/BC/BCInflowSupersonicFichSA_k.for"
              enddo 
           enddo 
          do k = ind_loop(5), ind_loop(6)-1
            do j = ind_loop(3), ind_loop(4)
