...
c     $Author: IvanMary $
c***********************************************************************
      subroutine bvbs_wallmodel_rhs(idir,lrhs, nstep,
     &                         neq_mtr, mobile_coef,
     &                         param_int, ind_loop,
     &                         param_real, c4, c5, c6,
     &                         x , y , z,
...
#include     "FastS/BC/BCWallModelRhs_i.for"
...
#include       "FastS/BC/BCWallModelRhs_i.for"              
          ...
          do j = ind_loop(3), ind_loop(4)
             ...
             ldp  = indven( ind_loop(2)+1 , j,  k )
             lmtr = indmtr( ind_loop(2)+1 , j,  k )
#include     "FastS/BC/BCWallModelRhsSA_i.for"
             do i = ind_loop(1), ind_loop(2)-1
               l    = inddm( i              , j,  k ) 
               ldp  = indven( ind_loop(2)+1 , j,  k )  !next rank
               lmtr = indmtr( ind_loop(2)+1 , j,  k )  !next rank
...
#include        "FastS/BC/BCWallModelRhs_j.for"
              enddo 
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                ...
                ldp  = indven( i, ind_loop(4)+1 , k )
                lmtr = indmtr( i, ind_loop(4)+1 , k )
#include        "FastS/BC/BCWallModelRhsSA_j.for"
              enddo 
            do j = ind_loop(3), ind_loop(4)-1
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
...
#include        "FastS/BC/BCWallModelRhs_k.for"
              enddo 
           enddo 
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                ...
                ldp  = indven( i,      j, ind_loop(6)+1  )
                lmtr = indmtr( i,      j, ind_loop(6)+1  )
#include        "FastS/BC/BCWallModelRhsSA_k.for"
              enddo 
           enddo 
          do k = ind_loop(5), ind_loop(6)-1
            do j = ind_loop(3), ind_loop(4)
