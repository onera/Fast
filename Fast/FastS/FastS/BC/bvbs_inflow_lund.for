...
c     $Author: IvanMary $
c***********************************************************************
      subroutine bvbs_inflow_lund(idir,lrhs, nstep,
     &                         neq_mtr, mobile_coef,
     &                         param_int, ind_loop,
     &                         param_real, c4, c5, c6,
     &                         x , y , z,
...
#include "FastS/param_solver.h"
     ...
     &       z( param_int(NDIMDX_XYZ) )
      ...
      INTEGER_E size_data, indbci, inc_bc(3) 
C...  Valeurs des grandeurs thermo imposees  (insert1)
      REAL_E lund_param(size_data) 
      REAL_E nut_planlund(size_data) 
      REAL_E t_planlund(size_data) 
      REAL_E w_planlund(size_data) 
      REAL_E v_planlund(size_data) 
      REAL_E u_planlund(size_data) 
      REAL_E rho_planlund(size_data) 
      REAL_E nut_in(size_data) 
      REAL_E t_in(size_data) 
      REAL_E w_in(size_data) 
      REAL_E v_in(size_data) 
      REAL_E u_in(size_data) 
...
#include       "FastS/BC/BC_nextrank_i.for"              
          ...
          do j = ind_loop(3), ind_loop(4)
             ...
             ldp  = indven( ind_loop(2)+1 , j,  k )
             lmtr = indmtr( ind_loop(2)+1 , j,  k )
#include     "FastS/BC/BCInflowLundSA_i.for"
             do i = ind_loop(1), ind_loop(2)-1
               l    = inddm( i              , j,  k ) 
               ldp  = indven( ind_loop(2)+1 , j,  k )  !next rank
               lmtr = indmtr( ind_loop(2)+1 , j,  k )  !next rank
#include       "FastS/BC/BC_nextrankSA_i.for"            
             enddo
          enddo
          enddo
...
#include        "FastS/BC/BCInflowLund_j.for"
              enddo 
...
#include        "FastS/BC/BC_nextrank_j.for"             
              enddo 
            enddo !j
          enddo !k
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                ...
                ldp  = indven( i, ind_loop(4)+1 , k )
                lmtr = indmtr( i, ind_loop(4)+1 , k )
#include        "FastS/BC/BCInflowLundSA_j.for"
              enddo 
            do j = ind_loop(3), ind_loop(4)-1
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
...
#include        "FastS/BC/BCInflowLund_k.for"
              enddo 
           enddo 
...
#include        "FastS/BC/BC_nextrank_k.for"                  
              enddo 
            enddo !j
          enddo !k
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                ...
                ldp  = indven( i,      j, ind_loop(6)+1  )
                lmtr = indmtr( i,      j, ind_loop(6)+1  )
#include        "FastS/BC/BCInflowLundSA_k.for"
              enddo 
           enddo 
          do k = ind_loop(5), ind_loop(6)-1
            do j = ind_loop(3), ind_loop(4)
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                ...
                ldp  = indven( i,      j, ind_loop(6)+1  )  !next rank
                lmtr = indmtr( i,      j, ind_loop(6)+1  )  !next rank
#include        "FastS/BC/BC_nextrankSA_k.for"                
              enddo 
            enddo !j
          enddo !k
       endif !param_int(NEQ)
