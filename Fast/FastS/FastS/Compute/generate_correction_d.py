...
select= open('template_correction_select_d.for','r')
...
fselecto = open(rep+'/correction_'+flux+'_select_d.for',"w")                  # ouvrir le fichier de sortie
    ...
    for eq in Model:
        for slope in TypeSlope:
            for typezone in TypeMesh:
                ...
                name_routine = 'corr_'+flux+ale1+eq+'_'+slope+'_'+typezone+'_d'
                select_out=[]
                if include == True:
...
if include == True:
    ...
    for l in lines_srcs:
        ...
        c+=1
    c_index +=1
    #print 'c_index sel', c_index, len(lines_srcs)
    lines_srcs_beg = lines_srcs[0:c_index]
    lines_srcs_end = lines_srcs[c_index:]
