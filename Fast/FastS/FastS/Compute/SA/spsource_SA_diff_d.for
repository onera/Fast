...
C
C  Differentiation of spsource_sa_diff in forward (tangent) mode:
...
C     $Author: IvanMary $
C***********************************************************************
      SUBROUTINE SPSOURCE_SA_DIFF_D(ndom, nitcfg, param_int, param_real
...
Canutild = max(rop(l,6)      , 1.e-27)
Cchi     = max(anutild*anulam, 1.e-7 ) 
CVersion F Renac
...
C
                  ...
                  ELSE
                    abs0 = -chi
                  END IF
CCALCUL DU TERME DE PRODUCTION
                  IF (abs0 .LT. 1.5) THEN
                    arg1d = 20.*chid*EXP(20.*chi)
                    arg1 = 1. + EXP(20.*chi)
                    chid = 0.05*arg1d/arg1
                    chi = 0.05*LOG(arg1)
                  ELSE IF (chi .LT. 0.) THEN
                    chi = 0.
                    chid = 0.0
                  END IF
C
...
COliver modification
                  stild = anutild*f2/(0.41*0.41*dist*dist)
                  IF (stild + 0.7*rot .GE. 0.) THEN
                    stild = rot + stild
                  ELSE
                    stild = rot*(1.+(0.7*0.7*rot+0.9*stild)/(0.9-2.*0.7*
     +                rot-stild))
                  END IF
C
...
CVersion F Renac: pas de limitation stild
...
COliver modification
              ...
              IF (stild + 0.7*rot .GE. 0.) THEN
                stild = rot + stild
              ELSE
                stild = rot*(1.+(0.7*0.7*rot+0.9*stild)/(0.9-2.*0.7*rot-
     +            stild))
              END IF
C
