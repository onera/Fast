...
#include "FastC/HPC_LAYER/SIZE_MIN.cpp"
  ...
  if (__NUMTHREADS__ <= core_per_socket)
    nb_socket = 1;

  // printf(" socket core %d %d \n", nb_socket, core_per_socket);

  /// obsolete (Ivan Mary)??
  FldArrayI tab_activ_core_per_socket(nb_socket);
  E_Int *activ_core_per_socket = tab_activ_core_per_socket.begin();

  E_Int count = __NUMTHREADS__;
  for (E_Int s = 0; s < nb_socket; s++)
  {
    if (count >= core_per_socket)
    {
      activ_core_per_socket[s] = core_per_socket;
      count -= core_per_socket;
    }
    else
      activ_core_per_socket[s] = count;
  }
  /// obsolete??
  ...
  {
    level_max = 0;
  } // Tous les schemas sauf dtlocal instationnaire
  else
  {
    E_Int denominateur_max = param_int[0][NSSITER] / 4;
    level_max = log(denominateur_max) / log(2);
  } // dtlocal instationnaire
  ...
  for (E_Int lev = 0; lev < level_max + 1; lev++)
  {
    ...
    FldArrayI tab_nijk(mxzone * 3 * 2);
    E_Int *ipt_nijk = tab_nijk.begin();
    FldArrayI tab_ndimdx(mxzone);
    E_Int *ipt_ndimdx = tab_ndimdx.begin();
    FldArrayI tab_nozone(mxzone);
    E_Int *ipt_nozone = tab_nozone.begin();
    FldArrayI tab_nosszone(mxzone);
    E_Int *ipt_nosszone = tab_nosszone.begin();

    // FldArrayI tab_size_task(mxzone);     E_Int*  size_task    = tab_size_task.begin();

    FldArrayI newtab_nijk(mxzone * 3 * 2);
    E_Int *ipt_nijk_new = newtab_nijk.begin();
    FldArrayI newtab_ndimdx(mxzone);
    E_Int *ipt_ndimdx_new = newtab_ndimdx.begin();
    FldArrayI newtab_nozone(mxzone);
    E_Int *ipt_nozone_new = newtab_nozone.begin();
    FldArrayI newtab_nosszone(mxzone);
    E_Int *ipt_nosszone_new = newtab_nosszone.begin();

    FldArrayF tab_hpccups(mxzone);
    E_Float *ipt_hpccups = tab_hpccups.begin();
    FldArrayF newtab_hpccups(mxzone);
    E_Float *ipt_hpccups_new = newtab_hpccups.begin();
    ...
    for (E_Int i = 0; i < nb_zones_dtloc; i++)
    {
      ...
      for (E_Int nd_subzone = 0; nd_subzone < nb_subzone; nd_subzone++)
      {
        ...
        if (ipt_hpccups[c] > CupsMax)
          CupsMax = ipt_hpccups[c];
    ...
    for (E_Int c = 0; c < Nbzones; c++)
    {
      ...
      for (E_Int c1 = 0; c1 < Nbzones; c1++)
      {
        ...
        if (float(ndimdx[0]) / hpc_cups[0] > ndimdx_max)
        {
          ndimdx_max = float(ndimdx[0]) / hpc_cups[0];
          c_tg = c1;
        }
    ...
    FldArrayI inddm_zone(Nbzones * 6);
    E_Int *ipt_inddm_zone = inddm_zone.begin();
    ...
    FldArrayI tab_remaind_sav(__NUMTHREADS__);
    E_Int *remaind_sav = tab_remaind_sav.begin();
    FldArrayI tab_remaind_pos(__NUMTHREADS__);
    E_Int *remaind_pos = tab_remaind_pos.begin();
    FldArrayI tab_remaind(__NUMTHREADS__);
    E_Int *remaind = tab_remaind.begin();
    FldArrayI tab_cells(__NUMTHREADS__);
    E_Int *cells = tab_cells.begin();
    FldArrayI tab_flu_i(__NUMTHREADS__);
    E_Int *flu_i = tab_flu_i.begin();
    FldArrayI tab_flu_j(__NUMTHREADS__);
    E_Int *flu_j = tab_flu_j.begin();
    FldArrayI tab_flu_k(__NUMTHREADS__);
    E_Int *flu_k = tab_flu_k.begin();
    FldArrayI tab_grain(__NUMTHREADS__);
    E_Int *grain = tab_grain.begin();

    FldArrayI tab_topo_lu(3);
    E_Int *topo_lu = tab_topo_lu.begin();
    FldArrayI tab_ind_dm_th(6);
    E_Int *ind_dm_th = tab_ind_dm_th.begin();

    FldArrayI tab_dim_i(__NUMTHREADS__);
    E_Int *dim_i = tab_dim_i.begin();
    FldArrayI tab_dim_j(__NUMTHREADS__);
    E_Int *dim_j = tab_dim_j.begin();
    FldArrayI tab_dim_k(__NUMTHREADS__);
    E_Int *dim_k = tab_dim_k.begin();

    FldArrayI tab_numa_socket(NBR_SOCKET);
    E_Int *numa_socket = tab_numa_socket.begin();
    ...
    for (E_Int c = 0; c < Nbzones; c++)
    {
      ...
      if (check > mx_omp_size_int && boom == 0)
      {
        boom = 1;
        check_size = check;
      }
      ...
      if (nstep == 1)
      {
        for (E_Int socket = 0; socket < NBR_SOCKET; socket++)
        {
          numa_socket[socket] = 0;
        }
      }
      ...
      E_Int size_k = 0;
      if (nijk[2] != 1)
        size_k = (nijk[2] + 1) * nijk[0] * nijk[1] * poids;
      ...
      for (E_Int th1 = 0; th1 < __NUMTHREADS__; th1++)
      {
        remaind[th1] = remaind_sav[th1];
      }

      E_Int rmax = remaind[remaind_pos[0]];

      E_Float marge = 1.005;
      if (rmax <= cells_tg_save / 8.)
      {
        marge = 1.16;
      }
      else if (rmax <= cells_tg_save / 4)
      {
        marge = 1.06;
      }
      else if (rmax <= cells_tg_save / 2)
      {
        marge = 1.04;
      }
      else if (rmax <= cells_tg_save / 1.5)
      {
        marge = 1.035;
      }
      ...
      else
      {
        ...
        if (size_c * 3 > cells_tg_save)
        {
          while (cells_loc < size_c && Nthreads < __NUMTHREADS__)
          {
            ...
            cells_loc += remaind[remaind_pos[Nthreads]] * marge;
            Nthreads += 1; // modif avril
          ...
          cells_tg_min = remaind[remaind_pos[Nthreads - 1]];
          th_min = -1;

          if (cells_tg_min != cells_tg_save)
          {
            th_min = remaind_pos[Nthreads - 1];
          }
          if (cells_tg_loc == cells_tg_min)
            th_min = -1;
        ...
        //{     E_Int target = cells_tg_save*(marge-1);
        {
          ...
          if (Nthreads > __NUMTHREADS__)
            Nthreads = __NUMTHREADS__;
          cells_loc = size_c / Nthreads;
          cells_tg_loc = cells_loc;
          cells_tg_min = cells_loc;
          th_min = -1;
        }
...
#include "FastC/HPC_LAYER/SIZE_MIN.cpp"
        ...
        while (search == 1)
        {
          ...
          for (E_Int dir = 0; dir < 3; dir++)
          {
            for (E_Int l = 0; l < topo_lu[dir]; l++)
            {
              ...
              if (dir == 0)
              {
                dim_i[l] = nijk[dir] / topo_lu[dir];
                if (l < nijk[dir] % topo_lu[dir])
                {
                  dim_i[l] += 1;
                }
              }
          ...
          */
          ...
          E_Float sign = 1.;
          if (res < 0)
            sign = -1.; // block trop petit % la cible
          ...
          for (E_Int dir = 0; dir < 3; dir++)
          {
            if (topo_lu[dir] != 1)
            {
...

              {
                ...
                if (dir == 0)
                  dim_i[topo_lu[dir] - 1] = nijk[dir] - (topo_lu[dir] - 1) * dim_i[0];
                else if (dir == 1)
                  dim_j[topo_lu[dir] - 1] = nijk[dir] - (topo_lu[dir] - 1) * dim_j[0];
                else
                  dim_k[topo_lu[dir] - 1] = nijk[dir] - (topo_lu[dir] - 1) * dim_k[0];
              }
            }
          } // loop dir
        ...
        for (E_Int k = 0; k < topo_lu[2]; k++)
        {
          for (E_Int j = 0; j < topo_lu[1]; j++)
          {
            for (E_Int i = 0; i < topo_lu[0]; i++)
            {
              ...
              if (socket >= nb_socket || socket >= NBR_SOCKET)
                socket = 0;
              ...
              if (i == topo_lu[0] - 1)
              {
                size_i = (dim_i[i] + 1) * dim_j[j] * dim_k[k];
              }
              if (j == topo_lu[1] - 1)
              {
                size_j = dim_i[i] * (dim_j[j] + 1) * dim_k[k];
              }
              if (k == topo_lu[2] - 1)
              {
                size_k = dim_i[i] * dim_j[j] * (dim_k[k] + 1);
              }
              if (nijk[2] == 1)
                size_k = 0;
        ...
        if (nstep == 1)
        {
          ...
          for (E_Int socket = 0; socket < NBR_SOCKET; socket++)
          {
            if (numa_socket[socket] > numa_max)
            {
              numa_max = numa_socket[socket];
              sock_tg = socket;
            }
          }
    ...
    // if ( (display==1 && nitrun ==1 ) || display ==2)
    {
      ...
      E_Float sum_c = 0., sum_i = 0., sum_j = 0., sum_k = 0.;
      for (E_Int th_current = 0; th_current < __NUMTHREADS__; th_current++)
      {
        if (fluk_tg != 0)
        {
...
PyObject *K_FASTC::distributeThreads(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "OOOlllll", &zones, &metrics, &dtloc, &nstep, &nitrun, &mx_omp_size_int, &display))
    return NULL;
#else
  if (!PyArg_ParseTuple(args, "OOOiiiii", &zones, &metrics, &dtloc, &nstep, &nitrun, &mx_omp_size_int, &display))
    return NULL;
#endif
